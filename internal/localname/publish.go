...
// Announcer starts and stops one advertisement process.
...
func (p *Publisher) Reconcile(ctx context.Context, records []Record, ip net.IP) error {
	...
	for _, name := range names {
		...
		pid, err := p.announce.Start(ctx, PublishArgs(next.Name, next.Port, ip))
		if err != nil {
			if first == nil {
				first = err
			}
		...
		p.live[name] = published{pid: pid, port: next.Port, ip: ip.String()}
	}
	return first
}
...
// PIDs returns the advertisement processes currently running.
