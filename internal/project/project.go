...
func Init(dir, name string) (Context, error) {
	...
	stateDir := filepath.Join(root, ".pier")
	if err := os.MkdirAll(stateDir, 0o700); err != nil {
		return Context{}, fmt.Errorf("create project state directory: %w", err)
	}
	if err := os.WriteFile(filepath.Join(stateDir, "id"), []byte(id+"\n"), 0o600); err != nil {
		return Context{}, fmt.Errorf("write project ID: %w", err)
	}
...
func contextForConfig(configPath string) (Context, error) {
	...
	id, err := os.ReadFile(filepath.Join(root, ".pier", "id"))
	if err != nil && !errors.Is(err, fs.ErrNotExist) {
		return Context{}, fmt.Errorf("read project ID: %w", err)
	}
	return Context{Root: root, ConfigPath: configPath, ID: strings.TrimSpace(string(id))}, nil
}
