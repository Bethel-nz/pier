...
- cloudflare is optional: a hostname on a Cloudflare zone the user owns, such as app.example.com. pier up serves it publicly through a Cloudflare Tunnel, using cloudflared (the user installs it and logs in once in the browser). Add it only when the user asks for their own domain; it is public on the internet.
